...
.DESCRIPTION
    ...
    from whatever is in the tree right now and labeled with a version you supply:
        ...
        build\artifacts\dev\Chronora-<version>-win-x64.zip
    ...
    Read the warning in the zip's README before installing one of these over a Chronora that
    someone relies on: the installer is the release installer, so it takes over the same
    install directory and the same shortcuts.

    It does not touch their data. Settings, templates, the run journal and any ExifTool they
    consented to live under %LOCALAPPDATA%\PaulTechGuy\Chronora, which the installer never
    goes near - the two trees are disjoint by design.
...
sent it to you. Installing it replaces any Chronora already on this machine
- same install folder, same shortcuts - and About will report the version
above. Your settings, templates, run history and any ExifTool you consented
to are kept, as they are across any upgrade: they live in a different folder
that the installer never touches.
...
try {
    ...
    Write-Host ''
    Write-Host "Chronora dev build $Version" -ForegroundColor White
    Write-Host ''
    ...
    # Four files, each of which has its own way of going missing and its own silent failure.
    #
    #   Chronora.pri, App.xbf   not copied by the plain publish targets - see
    #                           PublishWinUIResources in the app project. Without them the
    #                           app dies before its first window on an ms-appx:/// URI.
    #   e_sqlite3.dll           a self-contained publish can drop it; nothing breaks until
    #                           somebody opens History, days later.
    #   exiftool.json           the offline fallback for the manifest. Without it the
    #                           fallback exists in code and never fires, which only shows up
    #                           on a machine that cannot reach the Pages site - precisely
    #                           where it was supposed to help.
    #
    # CI asserts the same four. This checks the artifact that actually ships.
    ...
    # The licensing gate, on the artifact people actually receive.
    #
    # CI has this check, but CI runs it against its own publish on a GitHub runner - a zip
    # built on a developer machine never passes through it. This repository is Apache-2.0 and
    # ExifTool is GPL/Artistic, so a stray local copy riding along in the publish is a
    # licensing problem rather than a bug. exiftool.json is excluded by name because it is
    # ours: the manifest naming the version to fetch.
    ...
    if ($built -ne $Version) {
        Write-Failed
        throw "Asked for version '$Version' but the built Chronora.exe reports '$built'. The zip would be named one thing and About would say another."
    }
