@echo off
rem ---------------------------------------------------------------------------
rem  Removes Chronora. Double-click, or run from a prompt.
rem
rem  The usual way to uninstall is Settings ^> Apps ^> Installed apps ^> Chronora,
...
if not exist "%~dp0install\Uninstall.ps1" (
    ...
    echo.
    echo Uninstall Chronora from Settings ^> Apps ^> Installed apps instead.
    pause
    exit /b 1
)
