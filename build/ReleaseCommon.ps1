...
function Test-GateAncestor {
    ...
    # Exit 1 means "asked, and the answer is no". Anything else means the question could not
    # be asked at all - most often that origin/$Released or origin/$Development does not
    # exist, which git reports as 128. Treating those the same produced a confident and
    # completely wrong diagnosis: "something has been committed to master that is not on dev"
    # for a repository that simply had no remote branches yet.
    if ($result.ExitCode -notin @(0, 1)) {
        Write-Failed
        throw @"
Could not compare 'origin/$Released' with 'origin/$Development' (git exit $($result.ExitCode)).

Usually this means one of those remote branches does not exist yet. Push both and try again.
"@
    }
...
# docs\version.json - the file the in-app update check fetches from the Pages site.
#
# There is no Marqora equivalent, so this is Chronora's own. It is not documentation: it is a
# shipped API surface, and it is the only thing that tells an installed copy a new release
# exists. Three ways to get it wrong, all silent:
#
#   never updated  - every existing user is told they are up to date, for ever, and the
#                    release is invisible to everybody who already has the app
#   updated early  - users are told a version exists and handed a URL that 404s, because
#                    Pages serves from the promoted branch before the release is published
#   mismatched     - users are promised one version and given another
#
# Writing it is therefore part of the release, not an afterthought, and Test-GateReleaseManifest
# below refuses a release whose manifest disagrees with Directory.Build.props.
function Set-ReleaseManifest {
    param(
    ...
    $manifest = [ordered]@{
        ...
        url          = $Url
        notes        = $Notes
        publishedUtc = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:sszzz')
    }
...
function Test-GateReleaseManifest {
    ...
    if ("$($manifest.version)" -ne $Version) {
        Write-Failed
        throw @"
docs\version.json says '$($manifest.version)' but this release is '$Version'.

Users would be told the wrong version exists, or told nothing exists at all.
"@
    }

    Write-Done "says $Version"
}
...
function Test-GateTests {
    ...
    Write-Check 'tests'
    Invoke-Dotnet `
        -Arguments @('test', $Solution, '-c', $Configuration, '-warnaserror', '--nologo') `
        -FailureMessage 'Tests failed.' `
        -Stream:$Stream
    Write-Done 'passed'
}

# Two gates that came with this library are deliberately NOT here.
#
# Test-GateNetworkClaim guards Marqora's claim to be offline by default. Chronora does not
# make that claim - it fetches the ExifTool manifest and checks for updates - so inheriting
# the gate would fail the build for telling the truth. docs\Releasing.md says so explicitly.
#
# Test-GateHeaders shelled build\Add-FileHeaders.ps1, which does not exist here, so it would
# have failed every run forever. It is also redundant: .editorconfig sets
# file_header_template with IDE0073 as a warning, Directory.Build.props sets
# EnforceCodeStyleInBuild, and Test-GateBuild below already runs -warnaserror. A missing SPDX
# header is a build failure before it could ever be a gate failure.

# The gate docs\Releasing.md calls "the release gate", and it was missing from the pipeline
# entirely - the csproj's own check only proves the manifest carries SOME 64-hex hash, which
# cannot catch a hash that no longer matches what is upstream.
#
# That is the failure this project cares most about: a manifest that does not match the file
# at its URL refuses EVERY ExifTool install, for everyone, from the moment the release ships,
# and reports itself as a checksum error that reads like tampering.
function Test-GateExifToolManifest {
    param(
        [Parameter(Mandatory)][string] $ScriptPath,
        [switch] $Offline
    )

    Write-Check 'exiftool manifest'

    # -Verify is the whole point: it downloads the archive and hashes it. The offline form
    # only catches the placeholder. -Offline exists for -WhatIf, so a rehearsal does not pull
    # 11 MB every time somebody checks whether a release could start.
    $arguments = @('-NoProfile', '-File', $ScriptPath)
    if (-not $Offline) { $arguments += '-Verify' }
    ...
    if ($Offline) { Write-Done 'offline check only' } else { Write-Done }
}
...
function Get-RepoSlug {
    ...
    # -AllowFailure so that "there is no origin" is reported as that, rather than as the
    # bare string " (git exit code 2)" from the helper's own failure path.
    $remote = Invoke-Git -Arguments @('-C', $RepoRoot, 'remote', 'get-url', 'origin') -AllowFailure

    if ($remote.ExitCode -ne 0) {
        throw 'There is no origin remote, so there is no GitHub repository to release to.'
    }
...
function New-StagedRelease {
    ...
    $name = "Chronora-$Version-$RuntimeIdentifier"
    ...
    # The licence and the notices ship IN the archive, not just in the repository.
    #
    # app\ is a self-contained publish, so the zip is full of third-party binaries -
    # Microsoft, SQLitePCLRaw, Serilog. Apache-2.0 asks for the licence and the attribution
    # notices to travel with the distribution, and an archive carrying neither is the wrong
    # place for a project this careful about licensing to be casual.
    foreach ($file in @('LICENSE', 'THIRD-PARTY-NOTICES.md')) {
        $source = Join-Path $RepoRoot $file

        if (-not (Test-Path -LiteralPath $source)) {
            throw "$file is missing from the repository root; the archive must not ship without it."
        }
